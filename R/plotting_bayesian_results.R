...
# a bit hacky: changing the labels to probability notation in a rather clumsy way
labels <- gsub("(.*),(S1|S2/S3|S4)","P(\\2|\\1)",labels)
#show(labels)
Parameter=paste(VarName, "[", rawlabels, "]", sep="")

if(missing(MeanGuide)){
    return (data.frame(Parameter=Parameter, Label=labels))
}
else{
    ss2<-as.data.frame(MeanGuide[,"Mean"])
    ss2$Name <- rownames(ss2)
    meanvector <- unlist(sapply(Parameter,function(x){return(ss2[,1][which(ss2$Name==x)])}))
    meancol <- as.factor(ifelse(abs(meanvector)<MeanTreshold,-1,1))
    return (data.frame(Parameter=Parameter, Label=labels, meancol=meancol))
...
GetAllPlots <- function(varname.in.model, varname.in.df,  vaihe1.df, post, sumstats){
    cat("Generating plots for", varname.in.df, " -- this may take a while..","\n") 
    ##1. ilman interaktioita
    gen.ints <- GetLabelNamesForGgMcmc(varname.in.model,list(levels(as.factor(vaihe1.df[[varname.in.df]])),levels(as.factor(vaihe1.df$location3))),MeanGuide=sumstats)
    genplots <- list()
    locnames <- c("S1","S2_S3","S4")
    for(s in c(1:3)){
        cat(varname.in.df, " -- Ilman interaktioita -- ", locnames[s],"\n")
        regex <- paste("^",gsub("\\.","\\\\.",varname.in.model),"\\[\\d+,",s,sep="")
        genplots[[locnames[s]]] <- StyleGgmcmcComp(ggs_caterpillar(ggs(post, family=regex, par_labels=gen.ints), horizontal=T)  +  aes(color=meancol) )
    }
    #2. interaktiot
    intvarname <- ifelse(grepl("b\\.",varname.in.model),paste("b.lang.",varname.in.df,sep=""),paste("lang.",varname.in.df,sep=""))
    ia.ints <- GetLabelNamesForGgMcmc(intvarname,list(levels(as.factor(vaihe1.df$lang)), levels(as.factor(vaihe1.df[[varname.in.df]])),levels(as.factor(vaihe1.df$location3))), MeanGuide=sumstats)
    iaplots <- list()
    for(s in c(1:3)){
        ...
        iaplots[[locnames[s]]] <- StyleGgmcmcComp(ggs_caterpillar(ggs(post, family=regex, par_labels=ia.ints), horizontal=T)  +  aes(color=meancol) )
    }
